...
export function zodTextEssentials({fieldName = "Field", minLength = 2, maxLength = 150}:{fieldName?: string, minLength?: number, maxLength?: number} = {}) {
	return z
  .string()
  .min(minLength, { message: `${fieldName} must be at least ${minLength} characters long.` })
  .max(maxLength, { message: `${fieldName} must be at most ${maxLength} characters long.` })
...
export function zodEmail({fieldName = "Email Address", minLength = 5, maxLength = 100}:{fieldName?: string, minLength?: number, maxLength?: number} = {}){
	return z
...
export function zodPassword({fieldName = "Password", minLength = 8, maxLength = 256}:{fieldName?: string, minLength?: number, maxLength?: number} = {}){
  return z
  .string()
  .min(minLength, { message: `${fieldName} must be at least ${minLength} characters long.` })
  .max(maxLength, { message: `${fieldName} must be at most ${maxLength} characters long.` })
...
export function zodAlphaNumericSpace({fieldName = "Field", minLength = 2, maxLength = 32}:{fieldName?: string, minLength?: number, maxLength?: number} = {}){
  return z
  .string()
  .min(minLength, { message: `${fieldName} is required.` })
  .max(maxLength, { message: `${fieldName} must be at most ${maxLength} characters long.` })
  .regex(/^[a-zA-Z0-9 ]+$/, { message: `${fieldName} should only contain letters, numbers and spaces.` });
}

export function zodLargeText({fieldName = "Field", maxLength = 512}:{fieldName?: string, maxLength?: number} = {}) {
	return z.string().max(maxLength, { message: `${fieldName} must be at most ${maxLength} characters long.` });
}

export function zodAddress({fieldName = "Address", minLength = 5, maxLength = 150}:{fieldName?: string, minLength?: number, maxLength?: number} = {}){
	return z
  .string()
  .regex(/^[A-Za-z0-9 !"&'()+,\-./:;=\\_]+$/u, {
    message: `${fieldName} should only contain letters, numbers, spaces and some essential characters.`,
...
export function zod0To9({fieldName = "Field"}:{fieldName?: string} = {}){
  return z
  .coerce.number( {message: `${fieldName} must be a number from 0 to 9.`} )
...
export function zodRarity({fieldName = "Rarity"}:{fieldName?: string} = {}){
  return z
  .coerce.number( {message: `${fieldName} must be a number from 1 to 5.`} )
  .min(1, { message: `${fieldName} must be at least 1.` })
  .max(5, { message: `${fieldName} must be at most 5.` });
}

export function zodFile({fieldName = "File", fileTypes= ['image/jpeg']}:{fieldName?: string, fileTypes?: string[] } = {}){
  return z
  .file()
  .refine((file) => fileTypes.some(type => fileType(file, type)), {error: `${fieldName} must be a ${fileTypes.join(", ")}.` })
}
